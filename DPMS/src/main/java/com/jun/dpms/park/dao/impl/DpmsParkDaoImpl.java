...
public class DpmsParkDaoImpl implements IDpmsParkDao{
	...
	public int getTotalItem() {
		...
		return ((Number)this.getCurrentSession().createQuery("select count(*) from DpmsPark p where p.isCharge='·ñ' or p.isCharge=null").uniqueResult()).intValue();
	}

	@Override
	public int getHisTotalItem() {
		// TODO Auto-generated method stub
		return ((Number)this.getCurrentSession().createQuery("select count(*) from DpmsPark p where p.isCharge='ÊÇ'").uniqueResult()).intValue();
	}

	@Override
	public List<DpmsPark> findAll(int eachPage, int currentPage) {
		// TODO Auto-generated method stub
		Query q = this.getCurrentSession().createQuery(" from DpmsPark p where p.isCharge='·ñ' or p.isCharge=null");
	...
	public List<DpmsPark> findHisAll(int eachPage, int currentPage) {
		// TODO Auto-generated method stub
		Query q = this.getCurrentSession().createQuery("from DpmsPark p where p.isCharge='ÊÇ'");
		q.setMaxResults(eachPage);
		q.setFirstResult((currentPage-1)*eachPage);
		return q.list();
	}
