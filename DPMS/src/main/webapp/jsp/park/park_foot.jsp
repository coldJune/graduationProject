...
<div class="pagination-wrap">
 <ul class="pagination">
	<li><a href="findAllPark?currentPage=1">首页</a></li>
	<c:choose>
		<c:when test="${page.currentPage>5}">
			<li><a href="findAllPark?currentPage=${page.currentPage-5 }">&lt;&lt;</a></li>
		</c:when>
		<c:otherwise>
			<li><a href="findAllPark?currentPage=1">&lt;&lt;</a></li>
		</c:otherwise>
	</c:choose>
	<c:choose>
		<c:when test="${page.currentPage>1}">
			<li><a href="findAllPark?currentPage=${page.currentPage-1 }">&lt;</a></li>
		</c:when>
		<c:otherwise>
			...
			<c:forEach var="x" begin="1" end="${page.totalPage }">
				<c:choose>
					<c:when test="${x==page.currentPage}">
						<li class="active"><a href="findAllPark?currentPage=${x}" >${x}</a></li>
					</c:when>
					<c:otherwise>
						<li ><a href="findAllPark?currentPage=${x}">${x}</a></li>
					</c:otherwise>
				</c:choose>
			</c:forEach>
		</c:when>
	...
	<c:choose>
		<c:when test="${page.totalPage<=5}">
			<li><a href="findAllPark?currentPage=${page.totalPage }">&gt;&gt;</a></li>
		</c:when>
		<c:when test="${page.currentPage<=5}">
			<li><a href="findAllPark?currentPage=${page.currentPage+5 }">&gt;&gt;</a></li>
		</c:when>
	...
	<li><a href="findAllPark?currentPage=${page.totalPage}">末页</a></li>
	</ul>
	</div>
</body>
</html>